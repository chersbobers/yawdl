...
// yawdl.ohm
var yawdl_default = `YAWDL {\r
  Program = Block*\r
  Block   = Style | Script | UI | TT | Image | Paget | Import\r
  \r
  Style   = style_kw "{" Content_Brace "}"\r
  Script  = script_kw "{" Content_Brace "}"\r
  UI      = ui_kw "{" Content_Brace "}"\r
  TT      = title_kw "{" Content_Brace "}"\r
  Paget   = paget_kw "{" Content_Brace "}"\r
\r
  Import  = import_kw "(" quote Content_Paren quote ")"\r
  \r
  Image   = image_kw "(" quote Content_Paren quote "," quote Content_Paren quote "," number "," number ")"\r
  \r
  Content_Brace = (~("}" Lookahead) any)*\r
  Content_Paren = (~(quote | ")") any)*\r
  \r
  style_kw  = caseInsensitive<"style">\r
  script_kw = caseInsensitive<"script">\r
  ui_kw     = caseInsensitive<"ui">\r
  title_kw  = caseInsensitive<"title">\r
  image_kw  = caseInsensitive<"image">\r
  paget_kw  = caseInsensitive<"paget">\r
  import_kw = caseInsensitive<"import">\r
  \r
  number    = digit+\r
  quote     = "\\""\r
  Lookahead = style_kw | script_kw | ui_kw | title_kw | image_kw | paget_kw | import_kw | end\r
}`;

// engine.ts
var yawdlGrammar = grammar(yawdl_default);
window.Yawdl = {
  compile: (sourceCode) => {
